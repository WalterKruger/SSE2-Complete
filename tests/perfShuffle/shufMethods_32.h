...
#include "../../include/sseCom_parts/getMSB.h"
...
NOINLINE __m128i viaXorC_u32(__m128i toShuff, __m128i indexes) {
    __m128i a = _mm_shuffle_epi32(toShuff, _MM_SHUFFLE(0,0,0,0));
    __m128i b = _mm_shuffle_epi32(toShuff, _MM_SHUFFLE(1,1,1,1));
    __m128i c = _mm_shuffle_epi32(toShuff, _MM_SHUFFLE(2,2,2,2));
    __m128i d = _mm_shuffle_epi32(toShuff, _MM_SHUFFLE(3,3,3,3));
    ...
    # if 1
    __m128i selIfEven =  _either_i128(c, a, isInHi);
    __m128i selIfOdd =   _either_i128(d, b, isInHi);

    return _either_i128(selIfOdd, selIfEven, isInOdd);
    #else

    __m128i aPos = _mm_andnot_si128(isInHi, _mm_andnot_si128(isInOdd, a));
    __m128i bPos = _mm_andnot_si128(isInHi, _mm_and_si128(  isInOdd, b));
    __m128i cPos = _mm_and_si128(   isInHi, _mm_andnot_si128(isInOdd, c));
    __m128i dPos = _mm_and_si128(   isInHi, _mm_and_si128(  isInOdd, d));

    return _mm_or_si128(_mm_or_si128(aPos, bPos), _mm_or_si128(cPos, dPos));
    #endif
}
